...
public class GreenRowPanel extends JPanel implements PropertyChangeListener {
     ...
     * For increasing and decreasing the buttons I put boundaries, so that it can increase,
     * or decrease from those certain values.
     * For the slider, I just added a listener for it.
     * For the checkbox, I just added an event listener for it,
     * from there it would trigger, the text box listener in order for the edit to happen.
     */
    private void addListeners() {
        //DO not remove the following statement.
        myColor.addPropertyChangeListener(this);
        final ActionListener buttonIncrease = new ActionListener() {
            @Override
        ...
        myIncreaseButton.addActionListener(buttonIncrease);
        final ActionListener buttonDecrease = new ActionListener() {
            @Override
            public void actionPerformed(final ActionEvent theE) {
        ...
        myDecreaseButton.addActionListener(buttonDecrease);
        final ChangeListener changeSlider = new ChangeListener() {
            @Override
            public void stateChanged(final ChangeEvent theE) {
                myColor.setGreen(myValueSlider.getValue());
            }
        };
        myValueSlider.addChangeListener(changeSlider);

        myEnableEditButton.addActionListener(theEvent -> {
            myValueField.setEditable(myEnableEditButton.isSelected());
            myValueField.addActionListener(theNewEvent -> {
                try {
                    final int number = Integer.parseInt(myValueField.getText());
    ...
    public void propertyChange(final PropertyChangeEvent theEvent) {
        if (PROPERTY_GREEN.equals(theEvent.getPropertyName())) {
            myDecreaseButton.setEnabled(true); //needed to be added for decrease to work
